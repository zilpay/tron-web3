import type { TIP6963ProviderInfo, TIP6963ProviderDetail } from './types';
import { uuidv4 } from './uuid';

let providerInfo: TIP6963ProviderInfo | null = null;

function getBearbyIcon(): string {
...
export function getOrCreateProviderInfo(): TIP6963ProviderInfo {
  if (!providerInfo) {
    providerInfo = {
      uuid: uuidv4(),
      name: "Bearby Wallet",
...
export function announceProvider(provider: any): void {
  ...
  const info = getOrCreateProviderInfo();
  const detail: TIP6963ProviderDetail = {
    info,
    provider,
  };
...
export function setupTIP6963RequestListener(provider: any): void {
  if (typeof window === 'undefined') return;
